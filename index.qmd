...
tribble(~Option,~Organization,~Duration,~Hours,
  ...
  gt() |>
  tab_header(title="Table 1: Options to arrange an Internship within GIMA MSc")
```
...
tribble(~Step,~Deliverable,~Time,~Description,
  ...
  gt() |>
  tab_header(title="Table 2: The 7 steps that represent the work breakdown structure of the internship")
```